...
class Mattermost:
    def __init__(self):
        ...
        self.driver.login()
        self.channel_id = self.driver.channels.get_channel_by_name_and_team_name(self.team, self.channel)["id"]
...
def post_morning_message():
    ...
    post = mm.post("🍽️ Guten Morgen! Hier ist der Speiseplan für heute:", file_ids=[file_id])
    if post:
...
if __name__ == "__main__":
    ...
    schedule.every().day.at(MORNING_TIME).do(post_morning_message)
    schedule.every().day.at(REMINDER_TIME).do(post_reminder_message)

    for job in schedule.jobs:
        logging.info(f"⏰ Scheduled: {job.job_func.__name__} at {job.at_time.strftime('%H:%M')}")
    ...
    try:
        ...
        while True:
            ...
            if next_run and last_logged_next_time != next_run:
                formatted = next_run.strftime('%d.%m.%Y %H:%M:%S')
